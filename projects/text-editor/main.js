...
function onKeyDown(evt){
  ...
  if(!(SYMBOL_STATE in ta)){
    var state = O.obj();
    ta[SYMBOL_STATE] = state;

    state.pos = null;
    state.clipboard = null;

    state.history = [{type: 1, val: '', start: 0, end: 0}];
    state.historyIndex = 0;
  }

  var state = ta[SYMBOL_STATE];
  var {key} = evt;
  ...
  if(actionType === 0){
    ...
    switch(key){
      ...
      case 'c':
        ...
        state.clipboard = val.slice(start, end);
        break;
      ...
      case 'z':
        ...
        var {val, start, end} = history[--state.historyIndex];
        ta.value = val;
        setStart(start);
        setEnd(end);
        update = 0;
      ...
      case 'y':
        if(!ctrl) break;
        if(state.historyIndex === history.length - 1) break;
        var {val, start, end} = history[++state.historyIndex];
        ta.value = val;
        setStart(start);
        setEnd(end);
      ...
      case 'ArrowUp':
        ...
        if(state.pos === null) state.pos = pos - start;
        moveAbs(start - 1);
        moveAbs(getLineStart(1));
        moveAbs(find((c, dpos) => {
          if(c === '\n' || dpos === state.pos) return 0;
          return 1;
        }));
        removePos = 0;
        break;
  ...
  if(saveHistory){
    var {type, val, start, end} = history[state.historyIndex];
    ...
    if(val !== obj.val){
      ...
      if(history.length === MAX_HISTORY_SIZE || type === 0 && actionType === 0){
        history[state.historyIndex] = obj;
      }else{
        history.push(obj);
        state.historyIndex++;
      }
